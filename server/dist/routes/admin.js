...
adminRouter.post("/companies", async (req, res) => {
    ...
    const { name, address, contact_number, email, logo_url, is_active, is_headquarters, } = req.body;
    if (!name?.trim()) {
...
adminRouter.post("/employees", async (req, res) => {
    const { email, password, fullName, role, companyId, companyAccess, warehouseAccess, featureAccess, } = req.body;
...
adminRouter.put("/employees/:id", async (req, res) => {
    const { fullName, role, companyAccess, warehouseAccess, featureAccess, } = req.body;
    if (!fullName) {
...
adminRouter.post("/banks", async (req, res) => {
    const { bankName, currentAmount } = req.body;
    if (!bankName?.trim()) {
...
adminRouter.post("/transaction-settings", async (req, res) => {
    const { companyId, prefix, currentCounter } = req.body;
    if (!companyId) {
        return res.status(400).json({ error: "companyId is required" });
    }
...
adminRouter.post("/expenses", async (req, res) => {
    const { companyId, category, amount, description, expense_date } = req.body;
...
adminRouter.post("/account-titles", async (req, res) => {
    const { companyId, code, title, category } = req.body;
...
adminRouter.post("/disbursement-vouchers", async (req, res) => {
    const payload = req.body;
    ...
    try {
        ...
        `;
                ...
                if (disbursementAmount > balance) {
                    throw new Error(`Insufficient bank balance. Available: ${balance.toFixed(2)}, requested: ${disbursementAmount.toFixed(2)}`);
                }
                await tx.$executeRaw `
