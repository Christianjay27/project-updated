...
function hasIdentifierData(identifier: OcrIdentifier): boolean {
  return !!(
...
function parseVisionResponse(visionResult: any): { identifiers: OcrIdentifier[]; raw_text?: string } {
  const response: { identifiers: OcrIdentifier[]; raw_text?: string } = { identifiers: [] };
  const annotations = visionResult?.responses?.[0];
  if (!annotations) return response;
  ...
  let currentIdentifier: OcrIdentifier | null = null;
  ...
  for (const line of lines) {
    ...
    if (upper.includes("PRODUCT") && upper.includes(":") && !upper.includes("PRIME")) {
      ...
      if (match) currentIdentifier.product_id = match[1].trim();
      continue;
    }

    if (!currentIdentifier) currentIdentifier = {};

    if (upper.includes("MODEL") && upper.includes(":")) {
      const match = line.match(/[Mm]odel\s*:\s*(.+?)(?:\s*$)/);
      if (match) currentIdentifier.model = match[1].trim();
    }
  ...
  if (response.identifiers.length === 0) {
    const identifier: OcrIdentifier = {};
    const patterns = {
    ...
    for (const [key, pattern] of Object.entries(patterns)) {
      ...
      if (match?.[1]) {
        (identifier as Record<string, string>)[key] = match[1].trim();
      }
    }
...
async function runIdentifierOcr(base64ImageRaw: string) {
...
opsRouter.get("/banks", async (req, res) => {
  const rows = await prisma.$queryRaw<Array<{ id: string; bank_name: string }>>`
    SELECT id, bank_name
    FROM banks
    WHERE is_active = 1
...
opsRouter.get("/transactions/by-number/:transactionNumber", async (req, res) => {
  ...
  type ItemRow = {
    ...
    unit_price: number | string;
    total_price: number | string;
    edited_unit_price: number | string | null;
    product_identifier: string | null;
    model: string | null;
    mac: string | null;
    dev_id: string | null;
...
opsRouter.get("/purchase-orders/bootstrap", async (req, res) => {
  ...
  const [pos, suppliers, warehouses, products] = await Promise.all([
    prisma.$queryRaw<Array<{
      ...
      status: string;
      subtotal: number | string;
      landing_costs_total: number | string;
      total_amount: number | string;
    ...
    prisma.$queryRaw<Array<{ id: string; name: string }>>`
      ...
      ORDER BY name ASC
    `,
    prisma.$queryRaw<Array<{ id: string; name: string; company_id: string; company_name: string | null }>>`
      ...
      ORDER BY w.name ASC
    `,
    prisma.$queryRaw<Array<{ id: string; name: string; cost_price: number | string; sku: string | null }>>`
...
opsRouter.get("/warehouse-stock", async (req, res) => {
  ...
  const rows = await prisma.$queryRaw<Array<{ product_id: string; quantity: number | string }>>`
...
opsRouter.get("/purchase-orders/:id/details", async (req, res) => {
  ...
  const [poRows, itemRows, lcRows] = await Promise.all([
    prisma.$queryRaw<Array<{
      ...
      expected_date: Date | null;
      notes: string | null;
    }>>`SELECT id, company_id, supplier_id, warehouse_id, po_number AS order_number, expected_date, notes FROM purchase_orders WHERE id = ${poId} LIMIT 1`,
    prisma.$queryRaw<Array<{
      id: string;
      product_id: string;
      quantity: number | string;
    ...
    `,
    prisma.$queryRaw<Array<{ id: string; cost_type: string; amount: number | string; notes: string | null }>>`
...
opsRouter.post("/purchase-orders", async (req, res) => {
  const payload = req.body as {
    ...
    total_amount: number;
    created_by?: string;
    items: Array<{ product_id: string; quantity: number; unit_cost: number; total_cost: number }>;
...
opsRouter.post("/purchase-orders/:id/receive", async (req, res) => {
  ...
  type PoRow = { id: string; company_id: string; warehouse_id: string; po_number: string };
  ...
  `;
  if (items.length === 0) return res.status(400).json({ error: "No items found" });

  await prisma.$transaction(async (tx) => {
    for (const item of items) {
      const existing = await tx.$queryRaw<Array<{ id: string; quantity: number | string }>>`
        SELECT id, quantity
        FROM current_stock
...
opsRouter.get("/pos/bootstrap", async (req, res) => {
  ...
  const warehouses = await prisma.$queryRaw<Array<{ id: string; name: string; company_id: string; company_name: string | null }>>`
    SELECT w.id, w.name, w.company_id, c.name AS company_name
    FROM warehouses w
    LEFT JOIN companies c ON c.id = w.company_id
    ${warehouseFilter}
  ...
  const [products, categories, vouchers, banks] = await Promise.all([
    prisma.$queryRaw<Array<{ id: string; name: string; selling_price: number | string; sku: string | null; category_id: string | null; company_id: string }>>`
      SELECT id, name, selling_price, sku, category_id, company_id
      FROM products
      WHERE is_active = 1 AND company_id IN (${Prisma.join(companyIds)})
      ORDER BY name ASC
    `,
    prisma.$queryRaw<Array<{ id: string; name: string; company_id: string }>>`
    ...
    prisma.$queryRaw<Array<{
      ...
      ORDER BY code ASC
    `,
    prisma.$queryRaw<Array<{ id: string; bank_name: string; current_amount: number | string }>>`
...
opsRouter.get("/dashboard/summary", async (req, res) => {
  ...
  const [productsCountRows, warehousesCountRows, stockRows, txRows, recentRows, expRows, dvRows, inventoryRows, banksRows] = await Promise.all([
    ...
    `,
    prisma.$queryRaw<
      Array<{ product_id: string; quantity: number | string; low_stock_alert: number | string; product_name: string | null }>
    >`
      SELECT
        cs.product_id,
        cs.quantity,
      ...
      WHERE 1 = 1
        ...
        ${warehouseFilter}
    `,
    prisma.$queryRaw<Array<{ id: string; base_total: number | string; created_at: Date; warehouse_id: string }>>`
    ...
    >`
      ...
      LIMIT 5
    `,
    prisma.$queryRaw<Array<{ amount: number | string }>>`
      SELECT amount
      FROM expenses e
    ...
    prisma.$queryRaw<Array<{ amount: number | string }>>`
      ...
      WHERE DATE(dv.date) >= ${monthStart}
        ${companyFilterDvSql}
    `,
    prisma.$queryRaw<Array<{ quantity: number | string; selling_price: number | string | null; cost_price: number | string | null }>>`
  ...
  const productTotals = new Map<string, { name: string; qty: number; alert: number }>();
  ...
  const lowStock: Array<{ product_name: string; warehouse_name: string; quantity: number; low_stock_alert: number }> = [];
...
opsRouter.post("/pos/checkout", async (req, res) => {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const item of payload.items || []) {
      ...
      const stockRows = await tx.$queryRaw<Array<{ id: string; quantity: number | string }>>`
    ...
    if (payload.payment_method === "bank" && payload.bank_id) {
      const bankRows = await tx.$queryRaw<Array<{ current_amount: number | string }>>`
...
opsRouter.get("/inventory/bootstrap", async (req, res) => {
  ...
  const [warehouses, products, stockRows, movementRows, identifierRows] = await Promise.all([
    prisma.$queryRaw<Array<{ id: string; name: string; company_id: string }>>`
      ...
      ORDER BY w.name ASC
    `,
    prisma.$queryRaw<Array<{ id: string; name: string; sku: string | null; company_id: string }>>`
  ...
  const idMap = new Map<string, Array<{ product_identifier?: string; model?: string; mac?: string; dev_id?: string }>>();
...
opsRouter.get("/inventory/transfer-identifiers", async (req, res) => {
  const productId = String(req.query.productId || "");
  const warehouseId = String(req.query.warehouseId || "");
  if (!productId || !warehouseId) return res.json([]);
  const rows = await prisma.$queryRaw<Array<{ id: string; product_identifier: string | null; model: string | null; mac: string | null; dev_id: string | null }>>`
...
opsRouter.post("/inventory/movement", async (req, res) => {
  const payload = req.body as {
    ...
    notes?: string;
    created_by?: string;
    identifiers?: Array<{ product_identifier?: string; model?: string; mac?: string; dev_id?: string; warehouse_id?: string }>;
  };
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (payload.movement_type === "in") newQty = currentQty + Number(payload.quantity || 0);
    if (payload.movement_type === "out") newQty = Math.max(0, currentQty - Number(payload.quantity || 0));
    if (payload.movement_type === "adjustment") newQty = Number(payload.quantity || 0);
    ...
    for (const item of payload.identifiers || []) {
      if (!item.product_identifier && !item.model && !item.mac && !item.dev_id) continue;
      await tx.$executeRaw`
...
opsRouter.post("/inventory/transfer", async (req, res) => {
  const payload = req.body as {
    ...
    company_id: string;
    created_by?: string;
    selected_identifier_ids?: string[];
  };

  const ref = `TRF-${Date.now()}`;
  await prisma.$transaction(async (tx) => {
    const sourceRows = await tx.$queryRaw<Array<{ id: string; quantity: number | string }>>`
      SELECT id, quantity FROM current_stock
    ...
    const destRows = await tx.$queryRaw<Array<{ id: string; quantity: number | string }>>`
      SELECT id, quantity FROM current_stock
...
opsRouter.get("/products/bootstrap", async (req, res) => {
  ...
  const [products, categories, suppliers, units, warehouses, stockRows, identifierRows] = await Promise.all([
    ...
    prisma.$queryRaw<Array<{ id: string; name: string }>>`
      ...
      ORDER BY name ASC
    `,
    prisma.$queryRaw<Array<{ id: string; name: string; abbreviation: string | null }>>`
    ...
    prisma.$queryRaw<Array<{ id: string; name: string; company_id: string }>>`
      ...
      ORDER BY name ASC
    `,
    prisma.$queryRaw<Array<{ product_id: string; warehouse_id: string; quantity: number | string; warehouse_name: string | null }>>`
    ...
    `,
    prisma.$queryRaw<Array<{ product_id: string }>>`
  ...
  const warehouseIdsMap = new Map<string, string[]>();
  for (const s of stockRows) {
    stockTotalMap.set(s.product_id, (stockTotalMap.get(s.product_id) || 0) + Number(s.quantity || 0));
    if (!warehouseNameMap.has(s.product_id) && s.warehouse_name) warehouseNameMap.set(s.product_id, s.warehouse_name);
    const ids = warehouseIdsMap.get(s.product_id) || [];
    if (!ids.includes(s.warehouse_id)) ids.push(s.warehouse_id);
...
opsRouter.get("/products/warehouses", async (req, res) => {
  const companyId = String(req.query.companyId || "");
  const rows = await prisma.$queryRaw<Array<{ id: string; name: string; company_id: string }>>`
    SELECT id, name, company_id FROM warehouses
...
opsRouter.get("/products/:id/details", async (req, res) => {
  ...
  const [landingCosts, identifiers, variants, companyAssignments, warehouseAssignments] = await Promise.all([
    prisma.$queryRaw<Array<{ id: string; description: string; amount: number | string }>>`
      SELECT id, description, amount FROM landed_costs WHERE product_id = ${productId} ORDER BY created_at ASC
    `,
    prisma.$queryRaw<Array<{ id: string; product_identifier: string | null; model: string | null; mac: string | null; dev_id: string | null; warehouse_id: string | null }>>`
...
opsRouter.post("/products/save", async (req, res) => {
  const payload = req.body as {
    ...
    unit_id?: string | null;
    company_id?: string | null;
    landingCosts?: Array<{ cost_type: string; amount: number | string; notes?: string }>;
    identifiers?: Array<{ product_identifier?: string; model?: string; mac?: string; dev_id?: string; warehouse_id?: string }>;
    variants?: Array<{ sku: string; name: string; selling_price: number | string; cost_price: number | string }>;
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const lc of payload.landingCosts || []) {
      if (!lc.cost_type || !Number(lc.amount || 0)) continue;
      await tx.$executeRaw`
    ...
    for (const idf of payload.identifiers || []) {
      if (!idf.product_identifier && !idf.model && !idf.mac && !idf.dev_id) continue;
      await tx.$executeRaw`
